...
public class Roster implements Iterable<Person> {
	...
	/**
	 * @param location
	 *            the location of the roster
	 * @param type
	 *            the type of roster initialization
	 * @throws IOException
	...
	public String toTable() {
		...
		// TODO Make this print dynamically or given a set of attributes
		// TODO use String.format() here to improve code
		// TODO Move this to view package
		...
		if(columnLength[0] < "Name".length())
			columnLength[0] = "Name".length();
		if(columnLength[1] < "Days Present".length())
			columnLength[1] = "Days Present".length();
		...
		builder.append('|').append("Name").append(spaces.substring("Name".length(), columnLength[0])).append('|')
				.append("Days Present").append(spaces.substring("Days Present".length(), columnLength[1])).append('|')
				.append('\n');
		builder.append(mainDivider).append('\n');
		...
		for(Person member : members) {
			...
			builder.append('|').append(name).append(spaces.substring(nameLength, columnLength[0])).append('|')
					.append(daysPresent).append(spaces.substring(daysPresentLength, columnLength[1])).append('|')
					.append('\n');
		}
